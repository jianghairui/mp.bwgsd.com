<?php /*a:3:{s:70:"/var/www/caves.wcip.net/application/admin/view/funding/order_list.html";i:1568270068;s:58:"/var/www/caves.wcip.net/application/admin/view/layout.html";i:1566381559;s:65:"/var/www/caves.wcip.net/application/admin/view/public/footer.html";i:1566381559;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <form id="multi-check">
            ...
            <tr class="text-c userinfo">
                ...
                <td><span class="order-font-price"><?php echo htmlentities($li['total_price']); ?></span>元</td>
                ...
                <td class="td-manage" rowspan="3">
                    <span>
                        <a title="查看详情" href="javascript:;" onclick="detail('订单详情','<?php echo url("Funding/orderDetail",array("id"=>$li["id"])); ?>')">
                        <i class="Hui-iconfont">&#xe6df;</i>
                        </a>
                    </span>
                    <span id="send-btn">
                        <?php if($li['status'] == 1 && $li['refund_apply'] == 0): ?>
                        <a title="发货" href="javascript:;" onclick="order_send('订单发货','<?php echo url("Funding/orderSend",array("id"=>$li["id"])); ?>','600','400')"">
                        <i class="Hui-iconfont">&#xe669;</i>
                        </a>
                        <?php endif; ?>
                    </span>
            ...
            <tr>
                <td colspan="7">
                    <div style="display: inline-flex">
                        <?php if($li['type'] == 1): ?>
                        <div class="goods_pics" style="background-image: url('<?php echo htmlentities($qiniu_weburl); ?><?php echo htmlentities($li['pics'][0]); ?>');"></div>
                        <?php else: ?>
                        <div class="goods_pics" style="background-image: url('<?php echo htmlentities($qiniu_weburl); ?>public/jinsan.jpg');"></div>
                        <?php endif; ?>
                        <div style="margin: 0 10px;">
                            <div>活动来源: <span class="order-font-b"><?php echo htmlentities($li['funding_title']); ?></span></div>
                            <div>众筹商品: <span class="order-font">
                                <?php if($li['type'] == 1): ?>
                                    <?php echo htmlentities($li['goods_name']); else: ?>
                                    无偿
                                <?php endif; ?>
                            </span></div>
                        </div>
            ...
            <tr>
                <td colspan="7">
                    ...
                    <?php else: ?>
                    <span style="margin-left: 15px;">无偿订单，无需发货</span>
                    <?php endif; ?>
                </td>
            </tr>
            <?php endforeach; ?>
...
<script type="text/javascript">
    ...
    var url = '<?php echo url("Funding/orderList"); ?>' + '?<?php echo $page["query"];?>';
    ...
    $("#search-btn").click(function () {
        ...
        window.location.href = '<?php echo url("Funding/orderList"); ?>' + '?' + str;
    });
    ...
    function mod_address(obj,id,old_address) {
        layer.prompt({
            ...
            if(click_lock) {
                click_lock = false;
                $.ajax({
                    url:"<?php echo url('Funding/modAddress'); ?>",
                    type:'post',
                    dataType:'json',
    ...
    function  mod_price(obj,id,old_price) {
        layer.prompt({
            ...
            if(click_lock) {
                click_lock = false;
                $.ajax({
                    url:"<?php echo url('Funding/modPrice'); ?>",
                    type:'post',
                    dataType:'json',
    ...
    function refund(obj,id) {
        layer.confirm('确认要退款吗？',function(index){
            $.ajax({
                url: "<?php echo url('Funding/orderRefund'); ?>",
                type: 'POST',
                dataType: 'json',
                data:{id:id},
                success: function(data){
