<?php /*a:3:{s:68:"/var/www/caves.wcip.net/application/admin/view/xuqiu/xuqiu_list.html";i:1572948190;s:58:"/var/www/caves.wcip.net/application/admin/view/layout.html";i:1566381559;s:65:"/var/www/caves.wcip.net/application/admin/view/public/footer.html";i:1566381559;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <form id="multi-check">
            ...
            <tr class="text-c userinfo">
                ...
                <td>
                    <?php $image=unserialize($li['pics']);if(!empty($image)): ?>
                    <div style="width: 80px;height: 80px;background-image: url('<?php echo htmlentities($qiniu_weburl); ?><?php echo htmlentities($image['0']); ?>');background-position: center;background-repeat: no-repeat;background-size: cover"></div>
                    <?php else: ?>
                    无
                    <?php endif; ?>
                </td>
                ...
                <td>
                    <?php if($li['status'] == '0'): ?>
                    <a class="btn btn-link radius" style="text-decoration:none;" onClick="xuqiu_shenhe(this,<?php echo htmlentities($li['id']); ?>)" href="javascript:;" title="审核">审核</a>
                    <?php else: ?>
                ...
                <td>
                    ...
                    <?php endif; ?>
                    <a style="text-decoration:none" class="ml-5" onClick="pagefull('编辑需求','<?php echo url("Xuqiu/xuqiuDetail",array("id"=>$li["id"])); ?>')" href="javascript:;" title="编辑">
                    <i class="Hui-iconfont">&#xe6df;</i>
                    </a>
                    <a style="text-decoration:none" class="ml-5" onClick="xuqiu_del(this,'<?php echo htmlentities($li['id']); ?>')" href="javascript:;" title="删除">
                        <i class="Hui-iconfont">&#xe6e2;</i>
                    </a>
                </td>
...
<script type="text/javascript">
    ...
    var url = '<?php echo url("Xuqiu/xuqiuList"); ?>' + '?<?php echo $page["query"];?>';
    ...
    $("#search-btn").click(function () {
        ...
        window.location.href = '<?php echo url("Xuqiu/xuqiuList"); ?>' + '?' + str;
    });

    var click_lock = true;
    /*需求审核*/
    function xuqiu_shenhe(obj,id){
        layer.confirm('审核需求？', {
                btn: ['通过','拒绝','取消'],
                shade: false,
                closeBtn: 0
            ...
            function(){
                if(click_lock) {
                    click_lock = false;
                    $.ajax({
                        url:"<?php echo url('Xuqiu/xuqiuPass'); ?>",
                        type:'post',
                        dataType:'json',
                        data:{id:id},
            ...
            function(){
                layer.prompt({
                    ...
                    if(click_lock) {
                        click_lock = false
                        $.ajax({
                            url:"<?php echo url('Xuqiu/xuqiuReject'); ?>",
                            type:'post',
                            dataType:'json',
    ...
    /*需求-删除*/
    function xuqiu_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            $.ajax({
                url: "<?php echo url('Xuqiu/xuqiuDel'); ?>",
                type: 'POST',
                dataType: 'json',
                data:{id:id},
                success: function(data){
    ...
    function recommend(obj,id) {
        if(click_lock) {
            click_lock = false;
            $.ajax({
                url: "<?php echo url('Xuqiu/recommend'); ?>",
                type: 'POST',
                dataType: 'json',
                data:{id:id},
                success: function(data){
