<?php /*a:3:{s:66:"/var/www/caves.wcip.net/application/admin/view/user/user_list.html";i:1571385610;s:58:"/var/www/caves.wcip.net/application/admin/view/layout.html";i:1566381559;s:65:"/var/www/caves.wcip.net/application/admin/view/public/footer.html";i:1566381559;}*/ ?>
<!DOCTYPE HTML>
<html>
<head>
    <meta charset="utf-8">
        ...
        <div class="text-c">
            ...
            <span class="select-box inline" style="width: 120px;">
                <select name="role" id="role" class="select">
                    ...
                    <option value="0" <?php if($param['role'] === '0'): ?>selected<?php endif; ?>>普通用户</option>
                    <option value="1" <?php if($param['role'] === '1'): ?>selected<?php endif; ?>>博物馆</option>
                    <option value="2" <?php if($param['role'] === '2'): ?>selected<?php endif; ?>>设计师</option>
                    <option value="3" <?php if($param['role'] === '3'): ?>selected<?php endif; ?>>工厂</option>
                </select>
            </span>
            <span class="select-box inline" style="width: 120px;">
                <select name="role_check" id="role_check" class="select">
                    <option value="">全部</option>
                    <option value="1" <?php if($param['role_check'] === '1'): ?>selected<?php endif; ?>>审核中</option>
                    <option value="2" <?php if($param['role_check'] === '2'): ?>selected<?php endif; ?>>已认证</option>
                    <option value="3" <?php if($param['role_check'] === '3'): ?>selected<?php endif; ?>>未通过</option>
                </select>
            </span>
        </div>
    </section>
    ...
    <table class="table table-border table-bordered table-bg">
        ...
        <tbody>
            ...
            <tr class="text-c userinfo">
                ...
                <td>￥：<?php echo htmlentities($li['balance']); ?></td>
                <td>
                    <?php switch($li['sex']): case "0": ?>
                    <span>保密</span><?php break; case "1": ?>
                    <span>男</span><?php break; case "2": ?>
                    <span>女</span><?php break; default: endswitch; ?>
                </td>
                <td><?php echo htmlentities($li['tel']); ?></td>
                <td>
                    <?php switch($li['role']): case "0": ?>
                    普通用户<?php break; case "1": ?>
                    博物馆<?php break; case "2": ?>
                    设计师<?php break; case "3": ?>
                    工厂<?php break; default: endswitch; ?>
                </td>
                <td>
                    <span <?php if($li['fake']): ?>style="color:red"<?php endif; ?>><?php echo htmlentities($li['org']); ?></span>
                </td>
                <td>
                    <?php switch($li['role_check']): case "1": ?>
                ...
                <td class="td-status">
                    <?php switch($li['status']): case "1": ?>
                    <span class="label label-success radius">正常</span><?php break; case "2": ?>
                    <span class="label label-danger radius">已拉黑</span><?php break; default: endswitch; ?>
                </td>
                <td>
                    <?php echo htmlentities(date("Y-m-d H:i:s",!is_numeric($li['create_time'])? strtotime($li['create_time']) : $li['create_time'])); ?>
                </td>
                <td class="td-manage">
                    <?php if($li['status'] != 2): ?>
                    <a style="text-decoration:none" onClick="user_stop(this,<?php echo htmlentities($li['id']); ?>)" href="javascript:;" title="拉进黑名单">
                        <i class="Hui-iconfont">&#xe631;</i>
                    </a>
                    <?php else: ?>
                    <a style="text-decoration:none" onClick="user_start(this,<?php echo htmlentities($li['id']); ?>)" href="javascript:;" title="恢复">
                        <i class="Hui-iconfont">&#xe66b;</i>
                    </a>
                    <?php endif; ?>
                    <a title="查看详情" href="javascript:;" onclick="user_detail('用户信息','<?php echo url("User/userDetail",array("id"=>$li['id'])); ?>')" class="ml-5" style="text-decoration:none">
                    <i class="Hui-iconfont">&#xe6df;</i>
                    </a>
                </td>
            </tr>
...
<script type="text/javascript">
    ...
    var url = '<?php echo url("User/userList"); ?>' + '?<?php echo $page["query"];?>';
    ...
    $("#search-btn").click(function () {
        ...
        window.location.href = '<?php echo url("User/userList"); ?>' + '?' + str;
    });
    ...
    function user_stop(obj,id) {
        layer.confirm('确认要拉黑吗？',function(index){
            $.ajax({
                url:"<?php echo url('User/userStop'); ?>",
                type:'post',
                dataType:'json',
                data:{id:id},
    ...
    function user_start(obj,id) {
        layer.confirm('确认恢复吗？',function(index){
            $.ajax({
                url:"<?php echo url('User/userGetback'); ?>",
                type:'post',
                dataType:'json',
                data:{id:id},
                success:function(data) {
